                      ...
                      const std::map<std::string,std::string>& parameters)
{
  ...
  H0_ = plots_.Add_Histo("selection_0",100,0.0,5.0);
  H1_ = plots_.Add_Histo("selection_1",100,0.0,5.0);
...
bool user::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  {
      _P_e_pPTorderingfinalstate.clear();
      _P_aPTorderingfinalstate.clear();
      _P_dpPTorderingfinalstate.clear();
  }
  // Filling particle containers
  {
    for (UInt_t i=0;i<event.mc()->particles().size();i++)
    {
      if (isP__e_pPTorderingfinalstate((&(event.mc()->particles()[i])))) _P_e_pPTorderingfinalstate.push_back(&(event.mc()->particles()[i]));
      if (isP__aPTorderingfinalstate((&(event.mc()->particles()[i])))) _P_aPTorderingfinalstate.push_back(&(event.mc()->particles()[i]));
      if (isP__dpPTorderingfinalstate((&(event.mc()->particles()[i])))) _P_dpPTorderingfinalstate.push_back(&(event.mc()->particles()[i]));
    }
  }
  ...
  H0_->IncrementNEvents(__event_weight__);
  {
    ...
    {
    for (ind[1]=0;ind[1]<_P_e_pPTorderingfinalstate.size();ind[1]++)
    {
    ParticleBaseFormat q;
    q+=_P_aPTorderingfinalstate[ind[0]]->momentum();
    q-=_P_e_pPTorderingfinalstate[ind[1]]->momentum();
  ...
  H1_->IncrementNEvents(__event_weight__);
  {
    ...
    for (ind[0]=0;ind[0]<_P_aPTorderingfinalstate.size();ind[0]++)
    {
    for (ind[1]=0;ind[1]<_P_dpPTorderingfinalstate.size();ind[1]++)
    {
    ParticleBaseFormat q;
    q+=_P_aPTorderingfinalstate[ind[0]]->momentum();
    q-=_P_dpPTorderingfinalstate[ind[1]]->momentum();
    H1_->Fill(q.r(),__event_weight__);
    }
    }
  }
  }
