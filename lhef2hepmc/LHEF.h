...
struct PDFInfo {
   ...
   * Initialize default values.
   */
  PDFInfo(): p1(0), p2(0), x1(-1.0), x2(-1.0),
	     xf1(-1.0),  xf2(-1.0), scale(-1.0) {}

  /**
   * Create from XML tag.
   */
  PDFInfo(const XMLTag & tag): p1(0), p2(0), x1(-1.0), x2(-1.0),
			       xf1(-1.0),  xf2(-1.0), scale(-1.0) {
    tag.getattr("scale", scale);
    tag.getattr("p1", p1);
    tag.getattr("p2", p2);
...
public:
  ...
  /**
   * The name of the generator which produced the file.
   */
  std::string genname;

  /**
   * The version of the generator which produced the file.
   */
  std::string genversion;
...
public:
  ...
  double weight() const {
    ...
    if ( weights.empty() || weights[0].weights.empty() )
      return XWGTUP;
    else
...
private:
  ...
  void init() {
	...
	heprup.genname = tag.contents;
	tag.getattr("version", heprup.genversion);
      }
...
public:
  ...
  bool readEvent(HEPEUP * peup = 0) {
	...
	throw std::runtime_error("Found incomplete eventgroup tag in "
				 "Les Houches file.");
      if ( tags[0]->attr.find("nreal") != tags[0]->attr.end() )
	eup.subevents.nreal = std::atoi(tags[0]->attr["nreal"].c_str());
      if ( tags[0]->attr.find("ncounter") != tags[0]->attr.end() )
	eup.subevents.ncounter = std::atoi(tags[0]->attr["ncounter"].c_str());
    ...
    for ( int i = 0, N = tags.size(); i < N; ++i ) {
      ...
      if ( tag.name == "weight" ) {
	eup.weights.push_back(Weight(tag));
      }
...
public:
  ...
  void init() {
    ...
    file << "<generator";
    if ( heprup.genversion.size() )
      file << " version=\"" << heprup.genversion << "\"";
    file << ">" << heprup.genname << "</generator>" << std::endl;
  ...
  bool writeEvent(HEPEUP * peup = 0) {
    ...
    if ( heprup.xsecinfo.neve > 0 ) {
      ...
      for ( int i = 0, N = eup.weights.size(); i < N; ++i )
	eup.weights[i].print(file);
	...
	file << "</clustering>" << std::endl;	
      ...
      if ( eup.pdfinfo.xf1 > 0 ) eup.pdfinfo.print(file);
...
The classes has now been updated to handle the suggested version 2 of
this file standard as discussed at the <a href="http://www.lpthe.jussieu.fr/LesHouches09Wiki/index.php/LHEF_for_Matching">Les Houches workshop 2009</a>.
...
information defined in version 2 in the standard. The other two main
